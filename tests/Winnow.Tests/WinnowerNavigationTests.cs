...
public class WinnowerNavigationTests : TestBase
{
    [Fact]
    public void Update_WhenChildrenModified_ThrowsHelpfulException()
    {
        ...
        var ex = Should.Throw<InvalidOperationException>(() => saver.Update(orders));
    ...
    [Fact]
    public void Update_WithValidationDisabled_IgnoresModifiedChildren()
    {
        using var context = CreateContext();
        SeedCustomerOrders(context, 10, itemsPerOrder: 3);
        ...
        var saver = new Winnower<CustomerOrder, int>(context);
        var result = saver.Update(orders, new WinnowOptions
        {
    ...
    [Fact]
    public void Update_WithLoadedChildren_HandlesValidationFailuresCorrectly()
    {
        using var context = CreateContext();
        SeedCustomerOrders(context, 10, itemsPerOrder: 3);
    ...
    [Fact]
    public void Update_AfterCompletion_AllEntitiesDetached()
    {
        using var context = CreateContext();
        SeedCustomerOrders(context, 10, itemsPerOrder: 3);
    ...
    public void UpdateParentOnly_WithLoadedChildren_UpdatesOnlyParent()
    {
        ...
        var saver = new Winnower<CustomerOrder, int>(context);
        var result = saver.Update(ordersToUpdate, new WinnowOptions
        {
            ValidateNavigationProperties = false
        });
    ...
    [Fact]
    public void UpdateOrderItems_DirectlyUsingWinnower_WorksCorrectly()
    {
        using var context = CreateContext();
        SeedCustomerOrders(context, 10, itemsPerOrder: 3);
        ...
        var result = saver.Update(itemsToUpdate);
