...
public class ParallelWinnowerCompositeKeyTests : ParallelTestBase
{
    [Fact]
    public async Task InsertAsync_CompositeKeyEntities_AllInserted()
    {
        EnsureDatabaseCreated();
        SeedOrdersForOrderLines();

        var factory = CreateContextFactory();
        var saver = new ParallelWinnower<OrderLine, CompositeKey>(factory, maxDegreeOfParallelism: 2);
    ...
    [Fact]
    public async Task UpdateAsync_CompositeKeyEntities_AllUpdated()
    {
        EnsureDatabaseCreated();
        SeedOrdersForOrderLines();
        SeedOrderLines(6);
        ...
        var result = await saver.UpdateAsync(orderLines);
    ...
    [Fact]
    public async Task UpsertAsync_CompositeKeyEntities_ExistingUpdated()
    {
        EnsureDatabaseCreated();
        SeedOrdersForOrderLines();
        SeedOrderLines(4);
        ...
        var result = await saver.UpsertAsync(existing);
