...
- Built-in `WithDataAnnotations<TEntity>()` adapter that drives pre-validation from `System.ComponentModel.DataAnnotations` attributes declared on the entity. Per-type reflection cost is paid once at cache build time; each annotated property's getter is compiled to a direct expression delegate so the per-entity hot path is a linear walk over `(compiled getter, ValidationAttribute[])` pairs with no `PropertyInfo.GetValue` reflection per entity. The adapter also runs class-level `ValidationAttribute`s (e.g. `[CustomValidation]` placed on the type) via `GetValidationResult` and invokes `IValidatableObject.Validate(ValidationContext)` when the entity implements it — cross-field rules and attribute-driven failures land in the same `ValidationCollector`. `IValidatableObject` errors carry code `WINNOW_VALIDATABLE_OBJECT` so they can be distinguished from attribute-driven failures (which use the attribute type name).
- `ValidationOptions`, `ValidationError` (readonly record struct), `ValidationCollector` (ref struct with pooled buffer), `ValidationFailureBehavior` (`RecordAsFailure` / `Throw`), and `WinnowValidationException` carrying aggregated failures when `Throw` is selected. The failures-list constructor requires a non-empty list — an empty list is semantically incoherent and is rejected with `ArgumentException`. Standard .NET exception constructors `()`, `(string)`, and `(string, Exception)` are also provided for serialization and re-throw scenarios; those overloads expose an empty `Failures` collection. The per-entity record is a top-level `WinnowEntityFailure` in the `Winnow` namespace, matching the rest of the API's flat type layout (`ValidationError`, `InsertFailure`, `WinnowFailure<TKey>`).
...
- `ParallelWinnower` + `ValidationFailureBehavior.Throw` now records only the entities the validator rejected. Previously, a single invalid entity in a partition caused the orchestrator to mark every entity in that partition as failed (including valid siblings). The orchestrator now catches the partition's `WinnowValidationException`, records only the offending entities as `FailureReason.ValidationError` failures, and re-runs the partition with the survivors so they reach the database. Behaviour now matches the single-context `Winnower` path.
- `ParallelWinnower` + `ValidationFailureBehavior.Throw` now attributes survivor results to the correct entity. Previously, after the orchestrator caught a partition's `WinnowValidationException` and re-ran the survivor list, the strategy re-indexed the survivors `0..N-1` and `ResultMerger` then added the partition offset — so an `InsertedEntity.OriginalIndex` / `InsertFailure.EntityIndex` / `UpsertFailure.EntityIndex` could point at the wrong original entity. The orchestrator now carries a survivor-position → partition-position map through recovery and remaps result indices before the top-level merge.
- `ParallelWinnower` + upsert + `ValidationFailureBehavior.Throw` now reports `UpsertFailure.AttemptedOperation` and `UpsertFailure.IsDefaultKey` correctly for validation-failed entities. Previously the parallel recovery path hard-coded `AttemptedOperation = Insert` and `IsDefaultKey = false` for every failure; the single-context path was already correct. The parallel path now reads the primary-key CLR properties (reflection-only, no tracker attach) to classify the failure the same way the non-parallel path does.
