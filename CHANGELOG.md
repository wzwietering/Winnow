...
- **`ValidationCollector.Create()`** public factory and public `Dispose()` so external test code can exercise its own `ValidatorDelegate<TEntity>` via `using var c = ValidationCollector.Create();` without leaking the rented `ArrayPool<T>` buffer.
